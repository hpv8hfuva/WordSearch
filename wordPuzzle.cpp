...
int main (int argc,char* argv[]){
   ...
   string b = argv[2];
   ifstream help;
   help.open(b); //gets the number of rows and cols
   help >> rows;
   help >> cols;
   help.close();  
   bool result = readInGrid (b, rows, cols);
   // if there is an error, report it
   if ( !result ) {
       cout << "Error reading in file!" << endl;
       exit(1); // requires the <stdlib.h> #include header!
   }
   // Get a word (of length 10), starti g at position (2,2) in the
   ...
   file.close();
   file.open(a.c_str());
   hashTable* ans = new hashTable(cap); 
   while(getline(file,line)){
     ans->insert(line);
   ...
   file.close();
   timer t;
   t.start(); //checks the run time of the search 
   string printoutans;
   string r;
   for(int a = 0; a < rows ; a++){ // goes through every direction and displays the word and direction and coordinate.
     for(int b = 0; b < cols ; b++){
       for (int i = 0; i < 8; i++){
	 for(int c = 3; c< maxlength; c++){
	   r = getWordInGrid(a,b,i,c,rows,cols);
	   
	   if(ans->contains(r) && c <= r.size()){
	     if(i==0){
	       printoutans +=  "N (" +to_string(a)+ ", "+to_string(b) +"):       ";
	     }
	     else if(i==1){
	       printoutans +=  "NE(" +to_string(a)+ ", "+to_string(b) +"):       ";  
	     }
	     else if(i==2){
	       printoutans +=  "E (" +to_string(a)+ ", "+to_string(b) +"):       ";
	     }
	     else if(i==3){
	       printoutans +=  "SE(" +to_string(a)+ ", "+to_string(b) +"):       ";  
	     }
	     else if(i==4){
	       printoutans +=  "S (" +to_string(a)+ ", "+to_string(b) +"):       ";
	     }
	     else if(i==5){
	       printoutans +=  "SW(" +to_string(a)+ ", "+to_string(b) +"):       "; 
	     }
	     else if(i==6){
	       printoutans +=  "W (" +to_string(a)+ ", "+to_string(b) +"):       ";
	     }
	     else if(i==7){
	       printoutans +=  "NW(" +to_string(a)+ ", "+to_string(b) +"):       ";
	     }
	   count++;
	   printoutans += r + "\n";
   ...
   t.stop(); //stops the run time 
   cout << printoutans; // prints out the string of all the words found
   cout << maxlength << endl; //displays longest word
   cout << count << " words found" << endl; //displays number of words found
   cout << "Found all words in " << t.getTime() <<" seconds" << endl;//time
   return 0;
}
